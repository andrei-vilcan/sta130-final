...
ggplot(sim_q3, aes(x = prop_diff_q3)) +
  geom_histogram(binwidth = 0.005, fill="gray", color="black") +
  labs(x = "Difference in proportion of B&Es for H/LR neighborhoods,
       assuming no difference between groups") + 
...
sim_q3 %>% 
  ...
  summarise(pvalue_q3 = n() / repetitions3)
```
Since the p-value is less than 0.0001, it means that there is a low possibility that there is data as unusual as our observed value (test statistic) under the null hypothesis. Therefore, we have strong evidence against the null that there is no difference in the proportion of B&E offences between high-risk and low-risk neighborhoods.
