...
```{r}
b_and_e_top <- b_and_e_top %>% 
  mutate(BnE = ifelse(offence == "B&E", "Yes", "No")) 
ggplot(b_and_e_top, aes(x=BnE, y=prop)) +
  geom_boxplot() + theme_bw()

b_and_e_lower <- b_and_e_lower %>% 
  mutate(BnE = ifelse(offence == "B&E", "Yes", "No")) 
ggplot(b_and_e_lower, aes(x=BnE, y=prop)) +
  geom_boxplot() + theme_bw()
...
#heights %>% ggplot(aes(x=footLength, y=height)) + geom_point() +   geom_smooth(method="lm", se=FALSE) + theme_minimal()
